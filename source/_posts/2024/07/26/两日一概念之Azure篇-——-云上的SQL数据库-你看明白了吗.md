---
title: 两日一概念之Azure篇 —— 奇奇怪怪的SQL数据库,你看明白了吗
top: false
cover: false
toc: true
...
![](两日一概念之Azure篇-——-奇奇怪怪的SQL数据库-你看明白了吗/SQLs.png)

用一幅图简单说明这三个产品的不同：  
![](两日一概念之Azure篇-——-奇奇怪怪的SQL数据库-你看明白了吗/azure-sql-servers.png)
...
因此，图上三种产品按从左往右的顺序，用户接触到的底层的配置和维护工作量就越大，但可以使用的原版 SQL Server 的功能就越多、定制的自由度就越高。  
...
![](两日一概念之Azure篇-——-奇奇怪怪的SQL数据库-你看明白了吗/elastic-pools.png)
...
### 实例池（Instance Pool）
可能借鉴于Azure SQL databases 的弹性池，SQL managed instances 创建了实例池的纵向伸缩方案，同样将多个 instances 部署在同一个服务器上。此功能目前仅标记为 **preview(预览版)**。
