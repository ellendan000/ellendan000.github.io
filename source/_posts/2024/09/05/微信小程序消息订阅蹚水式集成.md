...
2. 微信平台提供的推送事件结果的[调试工具](https://developers.weixin.qq.com/apiExplorer?type=messagePush), 即使是按照文档说明 response 空字符串或者success，依然会显示错误信息“回包格式非法”。  
...
安全模式，不同的点是：  
1. 需要多加入 request body 中的 `Encrypt`，一共 4 项进行 sha1 计算签名 calc_signature。    
2. 验签时，使用的是 queryString 上的 `msg_signature` 值。  
...
**测试**    
当 GET API 开发完成，并发布到开发者服务器上之后，可以使用微信提供的[调试工具](https://developers.weixin.qq.com/apiExplorer?type=messagePush)的第一个**URL配置验证**对 GET API 进行验证和诊断。注意，这时还不需要去微信平台设置开发者服务器地址。  

**提前配置验证**    
在完成了“调试工具”的测试之后，可以考虑提前去微信平台配置开发者服务器地址（如果有测试环境，建议先配置成测试环境）。  
...
2. 为了便于微信调试工具进行调试，代码分支上可以支持 `Event`值为 "debug_demo" 的处理逻辑。如果 Event 等于 **debug_demo**，事件结果不读取 `List`，需要读取`debug_str`。  
```
...
```
- 微信提供的[调试工具](https://developers.weixin.qq.com/apiExplorer?type=messagePush)的第二个**请求构造**，可以按照工具表单上填写的 input 生成微信调用时的 queryString 和 request body（注意，不会发送真实的请求）。复制这些内容，可以用于本地开发测试。  
- 微信提供的[调试工具](https://developers.weixin.qq.com/apiExplorer?type=messagePush)的第三个**调试工具**，使用的前提条件：1. 提前在小程序管理后台配置好了开发者服务器地址；2. 实现了"debug_demo"分支逻辑的 POST API 已经发布到开发者服务器上。  
